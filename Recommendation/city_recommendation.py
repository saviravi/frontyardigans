...
def recommend_cities_nar(preference: YelpRecommendationCategories, limit=1,
                         exclude: list[City] = []) -> list[City]:
    cities = list(City)
    ...
    sorted_nars = sorted(nars,
                         key=lambda city_nar: city_nar[1][category_index_mapping[preference]],
                         reverse=True)
...
def recommend_cities_wnar(preference: YelpRecommendationCategories, limit=1,
                          exclude: list[City] = []) -> list[City]:
    cities = list(City)
    cities = [c for c in cities if c not in exclude]
    wnars = []
